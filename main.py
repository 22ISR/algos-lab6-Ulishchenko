...
def main():
    print("Игра 'Виселица'")
    print("Угадай слово по буквам, пока я не повесился!")
    
    COMMON_NOUNS = ["время", "человек", "год", "дело", "день", "рука", "раз", "город", "слово", "место"]
    attempts = 6
    used_letters = []
    
    while True:
        COMMON_NOUNS  = random.choice(COMMON_NOUNS)
        hidden_COMMON_NOUNS= ["_"] * len(COMMON_NOUNS)
        mistakes = 0
        
        while True:
            print("\nСлово:", " ".join(hidden_COMMON_NOUNS))
            print("Промахи:", mistakes)
            print("Юзал:", " ".join(used_letters))
            print(HANGMANPICS[mistakes])
            
            if "_" not in hidden_COMMON_NOUNS:
                print("Молодец(нет)! Ты спас меня!")
                break
                
            if mistakes >= attempts:
                print("Ты лох! Слово было:",COMMON_NOUNS)
                break
                
            guess = input("Введи буковку: ").lower()
          
            if len(guess) != 1 or not guess.isalpha():
                print("Нужно ввести одну буковку!")
                continue
            ...
            if guess in COMMON_NOUNS:
                print("Есть такая!")
                for i in range(len(COMMON_NOUNS)):
                    if COMMON_NOUNS[i] == guess:
                        hidden_COMMON_NOUNS[i] = guess
            else:
                print("Нет такой!")
                mistakes += 1
                
        again = input("Хочешь сыграть еще? (гоу/обойдусь): ").lower()
        if again != "гоу":
